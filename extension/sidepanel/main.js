// ===== Live Transcriber sidepanel: WS + chunked fallback + offline queue =====
// Fresh start each session, offline-first queue with drain-before-post,
// backend reachability checks, chronological render, dedup, exports.
...
const QUEUE_FLUSH_BATCH  = 8;

// Chronological render buffer
const REORDER_SECS = 12;
...
[setSourceTab,setSourcePick,setSourceMic].forEach(r=>{
  r.addEventListener('change',()=>{
    settings.source = setSourceTab.checked ? 'tab' : setSourcePick.checked ? 'pick' : 'mic';
    saveSettings();
  });
});
...
function toast(msg, kind='ok'){
  ...
  clearTimeout(toastT);
  toastT = setTimeout(()=>toastEl.className='', 2000);
}
...
function insertTranscriptLine(text, atSeconds){
  ...
  let idx=entries.findIndex(e=>t<e.t); if (idx===-1) idx=entries.length;
  entries.splice(idx,0,{t,text});
  persist();
  setButtons({
    start:false, pause:recording||wsMode, resume:!recording&&!wsMode&&stream, stop:!!stream,
...
function toSrt(){
  ...
  for (let i=0;i<entries.length;i++){
    ...
    const start=a.t||0, end=b?Math.max(start+1,(b.t||start+3)-0.2):start+3;
    out.push(`${i+1}\n${fmt(start)} --> ${fmt(end)}\n${a.text}\n`);
  }
  return out.join('\n');
}
...
async function enqueueChunk(blob, seq, t){
  try{
    ...
    if (cnt>=QUEUE_MAX_ITEMS){ const olds=await queueTake(Math.min(20,cnt)); await queueRemove(olds.map(o=>o.id)).catch(()=>{}); }
    await queueAdd({ blob, mime: blob.type||'audio/webm', seq, t });
    await refreshQueueCount();
    dbg('📥 enqueue (offline/backlog/drain)');
    startFlushLoop();
...
async function backendReachable(){
  try{
    const r=await fetchWithTimeout(`${BACKEND_URL}/health`, {method:'GET'}, HEALTH_TIMEOUT_MS);
    return r.ok;
  }catch{ return false; }
}

async function flushQueueOnce(){
  if (!(await backendReachable())) return false;
  const items = await queueTake(QUEUE_FLUSH_BATCH).catch(()=>[]);
  if (!items.length) return false;

  // oldest first by seq, then id
  items.sort((a,b)=>{
    const as=Number(a.seq||0), bs=Number(b.seq||0);
    return as===bs ? (a.id<b.id?-1:1) : as-bs;
  });
  ...
  for (const it of items){
    ...
    try{
      ...
      if (raw){
        ...
        dbg('…queued seg produced empty text (silence/dup)');
      }
      await queueRemove([it.id]).catch(()=>{});
    }catch(e){
      // leave item for next pass
      break;
    }
  }
  await refreshQueueCount();
...
async function drainQueueFully(){
  if (flushMode) return draining;
  flushMode=true;
  draining=(async()=>{
    try{
      while ((await queueCount())>0 && (await backendReachable())){
...
let wsRecorder=null, wsProbeInterval=null;
...
function wsConnect(){
  return new Promise((resolve,reject)=>{
    try{
      ...
      const to=setTimeout(()=>{ try{ws.close();}catch{} reject(new Error('WS connect timeout')); }, 2000);
...
function startRecorder(){
  ...
  R.mr.onstop=async()=>{
    ...
    R.chunks=[];
    if (blob){
      const backlog = await queueCount().catch(()=>0);
      const reachable = await backendReachable();
      if (!reachable || backlog>0 || flushMode){
        await enqueueChunk(blob, R.idx, typeof R.t==='number'?R.t:null);
        if (reachable) startFlushLoop();
      }else{
        ...
        try{
          ...
          if (raw){
            ...
            queueRender(raw, when);
          }else{ dbg('…all overlap (deduped/empty)'); }
        }catch{
...
// ---------- Render buffer ----------
let renderBuf=[], renderTimer=null, maxTSeen=-Infinity;
...
btnStart.addEventListener('click', async ()=>{
  try{
    setStatus('Starting…'); setButtons({start:false,pause:false,resume:false,stop:false,exportable:entries.length>0,canClear:entries.length>0});
    await startFresh(); // truly start from zero
    ...
    if (settings.preferWS){
      ...
      }catch{ setConn('WS unavailable → fallback'); toast('WS not ready, using fallback','warn'); startWsProbe(); }
    }

    recording=true; setConn('Connected'); setStatus(`Recording (fallback) — ${currentSourceLabel()}`);
    startRecorder(); if (offlineQueueCount>0) startFlushLoop();
  }catch(err){
...
btnStop.addEventListener('click', ()=>{
  clearWsProbe(); if (wsMode) wsStop();
  recording=false; abortAll();
  try{ if (stream) stream.getTracks().forEach(t=>t.stop()); }catch{}
  stream=null; stopTimer(); setStatus('Stopped'); setConn('Disconnected'); toast('Stopped','ok');
  setButtons({start:true,pause:false,resume:false,stop:false,exportable:entries.length>0,canClear:entries.length>0});
});
